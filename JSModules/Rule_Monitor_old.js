...
function start() {
  ...
  ipc.config.id = 'Rule_MonitorIPC';
ipc.config.retry = 1500;
ipc.config.silent = true;
ipc.connectTo('MQTTParserIPC', () => {
  ipc.of.MQTTParserIPC.on('connect', () => {
    ipc.of.MQTTParserIPC.emit('test', "The message we send2");
                       ...
                       //  myconsole.log("rule items are: " + node + "   " + nodePort + "    " + state );
                        ...
                        //  myconsole.log("rule items are2: " + node + "   " + nodePort + "    " + state );
                         	  ...
                         	  myconsole.log( node + "    " + nodePort + "    " + state);
                         	   try{ MQTTsocket.emit('deviceSwitch',node,nodePort,state,1,0);}catch(e){myconsole.dumpError(e)}
                         	    ipc.of.MQTTParserIPC.emit('deviceSwitch',node,nodePort,state,1,0);
