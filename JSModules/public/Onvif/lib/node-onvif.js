...
Onvif.prototype._discoverySendProbe = function(callback) {
	...
	this.udp.send(buf, 0, buf.length, this.PORT, this.MULTICAST_ADDRESS, function(error, bytes)  {
		this.discovery_retry ++;
		if(this.discovery_retry >= this.discovery_retry_max) {
			this.discovery_retry = 0;
			this.stopDiscovery();
		} else {
			this.discovery_timer = setTimeout(function() {
...
Onvif.prototype.startDiscovery = function(callback) {
	...
	this.udp.once('error', function(error)  {
		var err = new Error('Failed to initialize the object: ' + error.toString());
		callback(err);
	});

	this.udp.on('message', function(buf, device_info)  {
		this.OnvifSoap.parse(buf.toString(), function(error, result) {
			if(error) {
				return;
			}
			var type = '';
	...
	this.udp.bind(0, '0.0.0.0', function()  {
		try{this.udp.removeAllListeners('error');}catch(err){console.log(err)};
			try{this._discoverySendProbe(callback)}catch(err){console.log(err)};;
	});
};
