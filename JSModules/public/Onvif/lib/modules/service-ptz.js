...
*    - time_diff: ms
* ---------------------------------------------------------------- */
var OnvifServicePtz = function(params) {
	this.xaddr = '';
	this.user = '';
	this.pass = '';
...
OnvifServicePtz.prototype.getNodes = function(callback) {
	var soap_body = '<tptz:GetNodes/>';
	var soap = this._createRequestSoap(soap_body);

	mOnvifSoap.requestCommand(this.oxaddr, 'GetNodes', soap, function(error, result) {
		if(!error) {
			try {
				var d = result['data']['PTZNode'];
				if(!Array.isArray(d)) {
...
OnvifServicePtz.prototype.getNode = function(params, callback) {
	...
	if(err_msg = mOnvifSoap.isInvalidValue(params['NodeToken'], 'string')) {
		callback(new Error('The "NodeToken" property was invalid: ' + err_msg));
		return;
	}

	var soap_body = '';
	soap_body += '<tptz:GetNode>';
	soap_body += '<tt:NodeToken>' + params['NodeToken'] + '</tt:NodeToken>';
	soap_body += '</tptz:GetNode>';
	var soap = this._createRequestSoap(soap_body);

	mOnvifSoap.requestCommand(this.oxaddr, 'GetNode', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.getConfigurations = function(callback) {
	var soap_body = '<tptz:GetConfigurations/>';
	var soap = this._createRequestSoap(soap_body);

	mOnvifSoap.requestCommand(this.oxaddr, 'GetConfigurations', soap, function(error, result) {
		if(!error) {
			try {
				var d = result['data']['PTZConfiguration'];
				if(!Array.isArray(d)) {
...
OnvifServicePtz.prototype.getConfiguration = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GetConfiguration', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.getConfigurationOptions = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GetConfigurationOptions', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.getStatus = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GetStatus', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.continuousMove = function(params, callback) {
	...
	if(err_msg = mOnvifSoap.isInvalidValue(params['Velocity'], 'object')) {
		callback(new Error('The "Velocity" property was invalid: ' + err_msg));
		return;
	}
	...
	for(var i=0; i<klist.length; i++) {
		var k = klist[i];
		var v = params['Velocity'][k];
		if(err_msg = mOnvifSoap.isInvalidValue(v, 'float')) {
			callback(new Error('The "' + k + '" property was invalid: ' + err_msg));
			return;
		}
	}
	...
	if('Timeout' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['Timeout'], 'integer')) {
			callback(new Error('The "Timeout" property was invalid: ' + err_msg));
			return;
		}
	}
	...
	soap_body +=     '<tt:PanTilt x="' + params['Velocity']['x'] + '" y="' + params['Velocity']['y'] + '" space="http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace"></tt:PanTilt>';
	soap_body +=     '<tt:Zoom x="' + params['Velocity']['z'] + '" space="http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace"></tt:Zoom>';
	soap_body +=   '</tptz:Velocity>';
	if(params['Timeout']) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'ContinuousMove', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.absoluteMove = function(params, callback) {
	...
	if(err_msg = mOnvifSoap.isInvalidValue(params['Position'], 'object')) {
		callback(new Error('The "Position" property was invalid: ' + err_msg));
		return;
	}
	...
	if('Speed' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['Speed'], 'object')) {
			callback(new Error('The "Speed" property was invalid: ' + err_msg));
			return;
		}
		for(var i=0; i<klist.length; i++) {
			var k = klist[i];
			var v = params['Speed'][k];
			if(err_msg = mOnvifSoap.isInvalidValue(v, 'float')) {
				callback(new Error('The "' + k + '" property was invalid: ' + err_msg));
				return;
			}
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'AbsoluteMove', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.relativeMove = function(params, callback) {
	...
	if(err_msg = mOnvifSoap.isInvalidValue(params['Translation'], 'object')) {
		callback(new Error('The "Translation" property was invalid: ' + err_msg));
		return;
	}
	...
	soap_body +=   '</tptz:Translation>';
	if(params['Timeout']) {
		soap_body += '<xs:Timeout>PT' + params['Timeout'] + 'S</xs:Timeout>';
	}
	soap_body += '</tptz:RelativeMove>';
	var soap = this._createRequestSoap(soap_body);

	mOnvifSoap.requestCommand(this.oxaddr, 'RelativeMove', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.stop = function(params, callback) {
	...
	if('PanTilt' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['PanTilt'], 'boolean')) {
			callback(new Error('The "PanTilt" property was invalid: ' + err_msg));
			return;
		}
	}
	...
	if('Zoom' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['Zoom'], 'boolean')) {
			callback(new Error('The "Zoom" property was invalid: ' + err_msg));
			return;
		}
	}
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'Stop', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.gotoHomePosition = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GotoHomePosition', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.setHomePosition = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'SetHomePosition', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.setPreset = function(params, callback) {
	...
	if('PresetToken' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['PresetToken'], 'string')) {
			callback(new Error('The "PresetToken" property was invalid: ' + err_msg));
			return;
		}
	}
	...
	if('PresetName' in params) {
		if(err_msg = mOnvifSoap.isInvalidValue(params['PresetName'], 'string')) {
			callback(new Error('The "PresetName" property was invalid: ' + err_msg));
			return;
		}
	}

	if(!('PresetToken' in params) && !('PresetName' in params)) {
		callbackError('Either the "ProfileToken" or the "PresetName" property must be specified.');
		return;
	}
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'SetPreset', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.getPresets = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GetPresets', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.gotoPreset = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'GotoPreset', soap, function(error, result) {
		callback(error, result);
	});
};
...
OnvifServicePtz.prototype.removePreset = function(params, callback) {
	...
	mOnvifSoap.requestCommand(this.oxaddr, 'RemovePreset', soap, function(error, result) {
		callback(error, result);
	});
};
