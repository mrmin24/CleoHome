...
ipc.config.id = 'CleoHomeServerIPC';
ipc.config.retry = 1500;
ipc.config.silent = true;
ipc.connectTo('MQTTParserIPC', () => {
  ipc.of.MQTTParserIPC.on('connect', () => {
    ipc.of.MQTTParserIPC.emit('test', "The message we send");
  });
}); 
...
mySensorsocket.on('deviceStatusChange',function(NodeID,NodePort,State){
  ...
  // var evaluate = require('../JSModules/Rule_Items_Evaluate');
   if(State > 0){State = 1;}
     ...
     db.getdata('Items',{Select: 'Id,Item_Enabled_Value',whereClause:'Node_Id = ' + NodeID.toString() + ' AND Node_Port = ' + NodePort.toString()},function(err2,data_receive){
        // myconsole.log(data_receive);
         if(data_receive[0]){
                    //myconsole.log(data_receive);
                  var  ID = data_receive[0].Id;
...
mySensorsocket.on('sensorStatusChange',function(NodeID,NodePort,State,Type){
   ...
   //var evaluate = require('../JSModules/Rule_Items_Evaluate');
     ...
     db.getdata('Items',{Select: 'Id',whereClause:'Node_Id = ' + NodeID.toString() + ' AND Node_Port = ' + NodePort.toString()},function(err2,data_receive){
        // myconsole.log(data_receive);
         if(data_receive[0]){
                    //myconsole.log(data_receive);
                 var   ID = data_receive[0].Id;
...
MQTTsocket.on('sensorStatusChange',function(NodeID,NodePort,State,Type){
   ...
   //var evaluate = require('../JSModules/Rule_Items_Evaluate');
     ...
     db.getdata('Items',{Select: 'Id,Item_Current_Value,Time_Updated,Item_Type',whereClause:'Node_Id = ' + NodeID.toString() + ' AND Node_Port = ' + NodePort.toString()},function(err2,data_receive){
        // myconsole.log(data_receive);
         if(data_receive[0]){
                    //myconsole.log(data_receive);
                  var  ID = data_receive[0].Id;
                 ...
                 //  myconsole.log("Updated: " + State + " " + (currentValue - 0.3) + " " + (currentValue + 0.3) + " " + (timediff >= 5*60)  );
                  ...
                  //  myconsole.log(time);
                   db.getSQL("Update Items SET Item_Current_Value = " + State + " ,Time_Updated = '" + time + "' WHERE Id = " + ID,function(err2,data_receive2){
                               ...
                               db.update("Items",data3,function(err2,data_receive3){
                                       ...
                                       if(data_receive3){
                                          // myconsole.log(ID + " " + State);
                                           ...
                                           db.getdata('Items',{Select: 'Id,Item_Current_Value,Item_Enabled_Value',whereClause:'Node_Id = ' + NodeID.toString() },function(err2,data_receive4){
