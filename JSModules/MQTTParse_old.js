...
/*	ipc.serve(() => ipc.server.on('deviceSwitch', function(NodeId, NodePort, State, rulereq, timeOn) {
					...
					if (Array.isArray(NodeId)){
						...
						myconsole.log("mysensMQTT22 " + NodeId + " " + NodePort + " " + State + " " + rulereq);
					//	myconsole.log("MQTT IPC Test:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
...
//	var MQTTsocket = null;
	...
	//	myconsole.log("xxxxxxxxxxxxxxxxxxx " + message + " " + topic);
		...
		if (topic[topic.length - 1] == "LWT" && (jsonMessage == 'Online' || jsonMessage == 'Offline')) {
			...
			MQTTsocket.emit("nodeAlive", topic[topic.length - 2], jsonMessage);
		...
		if (node == "sonoff" && nodeReqActive == 0) {
			...
			db.getdata('Nodes', {
				...
				if (data_receive[0]) {
					...
					MQTTsocket.emit("newNode", ID, "", idReq);
					nodeReqActive = 0;
		...
		if (topic[topic.length - 1].indexOf("POWER") > -1) {
			...
			if (jsonMessage == "ON") {
				...
				MQTTsocket.emit("deviceStatusChange", node, trimmedStr, 1);
			}
			else if (jsonMessage == "OFF") {
				myconsole.log("MQTT sensor: Device updated " + node + " " + trimmedStr + " " + message);
				MQTTsocket.emit("deviceStatusChange", node, trimmedStr, 0);
			}
				...
				if (i == 0) {
					if (jsonMessage['StatusSTS']['POWER']) {
						...
						if (jsonMessage['StatusSTS']['POWER'] == "ON") {
							...
							MQTTsocket.emit("deviceStatusChange", node, 1, 1);
						}
						else if (jsonMessage['StatusSTS']['POWER'] == "OFF") {
							//	myconsole.log("MQTT sensor: Device updated " + node + " " + message);
							MQTTsocket.emit("deviceStatusChange", node, 1, 0);
						}
					...
					if (jsonMessage['StatusSTS']['POWER' + i.toString()]) {
						...
						if (jsonMessage['StatusSTS']['POWER' + i.toString()] == "ON") {
							...
							MQTTsocket.emit("deviceStatusChange", node, i, 1);
						}
						else if (jsonMessage['StatusSTS']['POWER' + i.toString()] == "OFF") {
							myconsole.log("MQTT sensor: Device updated " + node + " " + i + " " + message);
							MQTTsocket.emit("deviceStatusChange", node, i, 0);
						}
		...
		if (topic[topic.length - 1].indexOf("INFO2") > -1) {
			...
			MQTTsocket.emit("updateNodeStatus", node);
		}
		...
		if (topic[topic.length - 1].indexOf("SENSOR") > -1) {
			...
			if (jsonMessage.Temperature) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 0, jsonMessage.Temperature, 'Temp'); //update nodes
			...
			if (jsonMessage.Humidity) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 1, jsonMessage.Humidity, 'Humidity');
			...
			if (jsonMessage.Light) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 2, jsonMessage.Light, 'Light');
			...
			if (jsonMessage.Noise) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 3, jsonMessage.Noise, 'Noise');
			...
			if (jsonMessage.AirQuality) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 4, jsonMessage.AirQuality, 'AirQual');
				...
				db.getdata('Items', data, function(err, result) {
					...
					if (err) {
						...
						if (result[0].Node_Id == node) {
							...
							MQTTsocket.emit("sensorStatusChange", node, 1, jsonMessage.Counter1, 'RainRate'); //update node
			...
			if (jsonMessage.DS18x20.DS1.Temperature) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 99, jsonMessage.DS18x20.DS1.Temperature, 'Temp'); //update node
			}
		...
		if (topic[topic.length - 1].indexOf("ENERGY") > -1) {
			...
			if (jsonMessage) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 99, jsonMessage.Power, 'Power');
		...
		if (topic[topic.length - 1].indexOf("STATE") > -1) {
			...
			if (jsonMessage['Wifi']['RSSI'] >= 0) {
				...
				MQTTsocket.emit("sensorStatusChange", node, 98, jsonMessage['Wifi']['RSSI'], 'RSSI');
			}
		...
		if (topic[topic.length - 1] == "cmd") { //if command topic is received check the command
			...
			if (command == "set") {
				...
				MQTTsocket.emit("deviceStatusChange", nodeID, port, value);
		...
		
			{
				...
				db.getdata('Items', {
					...
					if (data_receive[0]) {
						...
						MQTTsocket.emit("newNode", ID, IP, idReq);
